...
def analyse_hard_cases(X_test, y_test, pred, genotype_col="vkorc1",save=False):
    ...
    if save: 
       ...
       comparison.to_csv(RESULTS_DIR/"comparison_of_hard_cases.csv", index=False)
    ...
    if genotype_col in df.columns:
        ...
        if save: # only save if you ask for it
        
         Path("results").mkdir(parents=True, exist_ok=True)
         genotype_comparison.reset_index().to_csv( f"results/genotype_comparison_{genotype_col}.csv",
         index=False
)
